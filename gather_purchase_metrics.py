...
def determine_layers_out(input_df, purchase_row):
    ...
    describe_network_interactions(temp_df, purchase_row)
...
def describe_network_interactions(temp_df, purchase_row):
    temp_df['full_date'] = pd.to_datetime(temp_df['full_date'])
    temp_df.sort_values(by='full_date', inplace=True)
    mean_time_between  = temp_df['full_date'].diff().mean()
    print('Mean time between interactions {}'.format(mean_time_between))
    ...
    print('Average dest count {}'.format(temp_df['Destination'].value_counts().mean()))
    print('Average source count {}'.format(temp_df['Source'].value_counts().mean()))
...
def look_at_size_of_network_X_layers_out(input_df, purchase_row, layers):
    ...
    time_filtered_df = time_filter_df(output_df, seconds_in_five_days, seconds_in_five_days, purchase_row['TimeStamp'])
    ##TODO
    source_dest = (time_filtered_df['Source'].isin([purchase_row['Source']])) | (time_filtered_df['Destination'].isin([purchase_row['Destination']]))
    dest_source = (time_filtered_df['Source'].isin([purchase_row['Destination']])) | (time_filtered_df['Destination'].isin([purchase_row['Source']]))
    temp_df = time_filtered_df[source_dest | dest_source]
    #temp_df = output_df[source_dest]
